...
def main():
    parser = argparse.ArgumentParser(description="Evaluate a causal language model adapter on a cloze dataset.")
    ...
    base_model = AutoModelForCausalLM.from_pretrained(
        args.model_name,
        torch_dtype=torch.bfloat16,  # or torch.float16 depending on your env
        device_map="auto"  # or .to(device) if only using 1 GPU
    )
